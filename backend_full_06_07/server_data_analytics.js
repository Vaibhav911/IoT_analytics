...
//   console.log("req received is " + JSON.stringify(req));
    ...
    {
        var attributes = await get_sensor_attributes('THL');
        var data = await  get_sensor_data(sensorIds, new Date(startTime), new Date(endTime), frequency, attributes, sensorSet);  
        console.log('data is line 90 ' + JSON.stringify(data.length))
...
async function get_sensor_data(sensorIds, start_time, end_time, frequency, attributes, sensorSet)
{
    ...
    var dailyDataArray = [];
    var hourlyDataArray = [];
    await Year_Schema.find({sensorId: sensorIds,
    ...
    await Month_Schema.find({sensorId: sensorIds,
    ...
    await Day_Schema.find({sensorId: sensorIds,
    ...
    await Hour_Schema.find({sensorId: sensorIds,
    ...
    return [frequency, attributes, sensorSet, start_time, end_time, annualDataArray, monthlyDataArray, dailyDataArray, hourlyDataArray];
}

async function get_sensor_attributes(sensorType)
{
    var attributeArray = [];
    await Sensor_Attributes_Schema.find({type: sensorType}, (err, sensorAttributes) => {
        if (err)
        {
        ...
        {
            console.log('attributes are ' + JSON.stringify(sensorAttributes[0]))
